...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const embed = new EmbedBuilder()
            .setTitle('🔥 Promoção Global da Loja Atualizada!')
            .setColor('#F43F5E')
            .setDescription(pct > 0 
                ? `✅ A promoção global da loja foi definida para **${pct}% OFF**!\n\nEste desconto é aplicado automaticamente a **TODOS** os itens do catálogo (Skins, Chromas, Passes, Pacotes, Campeões, Emotes, Ícones, Sentinelas, etc.).`
                : `✅ A promoção global da loja foi **desativada** (0% OFF). Os itens serão exibidos pelo preço padrão.`
            )
            .setFooter({ text: 'Kitsune Store • Promoção Global' })
            .setTimestamp();
