        ...
        if (interaction.isChatInputCommand()) {
            ...
            if (interaction.customId === 'abrir_loja') {
                ...
                const embed = formatEmbed(new EmbedBuilder(), interaction.client)
                    ...
                    .setColor('#F43F5E')
                    .setDescription('Bem-vindo ao **Gerenciador de Embeds**! Aqui você pode customizar todos os textos, títulos, descrições, cores, imagens e rodapés de todas as mensagens do bot (incluindo todo o fluxo do `/login`).\n\nSelecione a embed que deseja editar no menu abaixo:');

                const menu = new ActionRowBuilder().addComponents(
                    new StringSelectMenuBuilder()
                        .setCustomId('menu_embed_select')
                        .setPlaceholder('Selecione uma embed para editar...')
                        .addOptions([
                            { label: 'Login (Passo 1 - Inicializando)', description: 'Embed de carregamento 1 do /login', value: 'login_loading_1', emoji: '⏳' },
                            { label: 'Login (Passo 2 - Token Atualizado)', description: 'Embed de carregamento 2 do /login', value: 'login_loading_2', emoji: '☑️' },
                            { label: 'Login (Passo 3 - Amigos Obtidos)', description: 'Embed de carregamento 3 do /login', value: 'login_loading_3', emoji: '👥' },
                            { label: 'Login Sucesso (Painel da Conta)', description: 'Embed final de sucesso do /login com informações da conta', value: 'login_success', emoji: '🛡️' },
                            { label: 'Login Sessão Expirada', description: 'Embed vermelha exibida quando a conta expirou', value: 'login_expired', emoji: '🔴' },
                            { label: 'Dashboard RP / BE', description: 'Embed exibida ao clicar no botão RP do /login', value: 'dashboard_rp', emoji: '🪙' },
                            { label: 'Dashboard Account Info', description: 'Embed exibida ao clicar no botão Account do /login', value: 'dashboard_account', emoji: 'ℹ️' },
                            { label: 'Dashboard Friends List', description: 'Embed exibida ao clicar no botão Friend do /login', value: 'dashboard_friends', emoji: '🫂' },
                            { label: 'Painel Boas-Vindas da Loja (/ticket)', description: 'Mensagem inicial do painel de vendas no chat', value: 'ticket_welcome', emoji: '✉️' },
                            { label: 'Autenticação de Região da Loja', description: 'Menu de seleção de região ao iniciar compra', value: 'store_authentication', emoji: '🌍' },
